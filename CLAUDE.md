...
- `usecases/` — orchestration. `BookingsUseCase` is the interesting one: it branches on `EventType` (`SEATED` locks seats via `get_seats_by_ids_for_update` + atomic `mark_seats_as_unavailable`; `TIERED` uses atomic `decrement_available_tickets`). Cancellation reverses both.
